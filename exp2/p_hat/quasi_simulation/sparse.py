...
def _bsr_ensure_fits_with_grad(bsr: BsrMatrix, nrow: int = None, nnz: int = None):
    if nrow is None:
        nrow = bsr.nrow
    if nnz is None:
        nnz = bsr.nnz
    ...
    if bsr.columns.size < nnz:
        bsr.columns = wp.empty(shape=(nnz,), dtype=int, device=bsr.columns.device)
    if bsr.values.size < nnz:
        bsr.values = wp.empty(shape=(nnz,), dtype=bsr.values.dtype, device=bsr.values.device,requires_grad=True)
...
def bsr_set_from_triplets_with_grad(
    dest: BsrMatrix[BlockType[Rows, Cols, Scalar]],
    rows: "Array[int]",
    columns: "Array[int]",
    values: "Array[Union[Scalar, BlockType[Rows, Cols, Scalar]]]",
...
def _bsr_mv_kernel(
    ...
    if alpha != scalar_zero:
        ...
        for block in range(beg, end):
            v += A_values[block] * x[A_columns[block]+x_offset]
        v *= alpha

    if beta != scalar_zero:
        v += beta * y[row+y_offset]
...
def bsr_mv(
    ...
    if A.ncol+x_offset > x.shape[0]:
        raise ValueError("Number of columns of A + offset of x > number of rows of x")
    if A.nrow+y_offset > y.shape[0]:
        raise ValueError("Number of rows of A + offset of y > number of rows of y")
    ...
    wp.launch(
        ...
        device=A.values.device,
        dim=A.nrow,
        inputs=[alpha, A.offsets, A.columns, A.values, x, x_offset,beta, y, y_offset]
    )
