...
class Cloth: 
    ...
    def __init__(self):
        ...
        self.warp_x_init = wp.from_torch(self.x_cpu.to('cuda:0'),dtype=wp.vec3f)
        self.x_init = self.x_cpu.to('cuda:0')
        #面数据
        ...
        self.edge_index = torch.zeros((2,self.res_x*(self.res_y+1)+self.res_y*(self.res_x+1)+self.res_x*self.res_y),dtype=torch.long,requires_grad=False)
        ...
        for i in range(self.edge.shape[0]):
            self.edge_index[0][i] = self.edge[i][0]
            self.edge_index[1][i] = self.edge[i][1]
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            v_torch =(torch.rand(self.N_verts,3)-0.5)*0.05
            ...
            for frame in range(frames):
                ...
                print(self.dev_delta_x)
                # x_torch =(torch.rand(self.N_verts,3,device='cuda:0')-0.5)*torch.mean(wp.to_torch(delta_x_order))*6.0
                #train component
                for t in range(train_times):
                    input_x = torch.cat((wp.to_torch(self.dev_inertia_x),self.x_init),dim=1)
                    delta_x = net(input_x,self.edge_index)
                    loss = torch.mean((delta_x.view(-1,3)-wp.to_torch(self.dev_delta_x))**2)
                    L=loss.item()
                    loss.backward()
                    ...
                    print('[train] : loss',L)
            if epoch%10 == 0:
                print("[train epoch] : ",epoch,"  ,loss : ",L)
